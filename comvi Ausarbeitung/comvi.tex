...
We ended up using the haralick textural features \todo{mail: reference} . The haralick features work with a grey level correlation matrix (GCM). The GCM for a given greyscale image is defined as such:
...
Alternatively one can also use the tamura texture features \todo{mail: reference}. the tamura features are different to the haralick featues because they \todo{expand tamura}
...
Main advantages of hierarchical agglomerative clustering is the hiararchy of clusters that it creates and its stabililty over multiple executions of clustering. Its main weakness is on the other hand relatively high time complexity that is in worst case $O(n^3)$. 
...
In the early stages of the 0projcet we intended to use the DBSCAN clustering algorithm. 
...
pointing Stick with two buttons (1 on each side), which was tracked by 14 cameras
using an optical tracking system from NaturalPoint. 
All the transformations were synchronized over all views, while the interaction is running.
